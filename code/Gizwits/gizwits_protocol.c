...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagSPK)
    {
        info->event[info->num] = EVENT_SPK;
        info->num++;
        dataPoints->valueSPK = gizStandardDecompressionValue(SPK_BYTEOFFSET,SPK_BITOFFSET,SPK_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagJD)
    {
        info->event[info->num] = EVENT_JD;
        info->num++;
        dataPoints->valueJD = gizStandardDecompressionValue(JD_BYTEOFFSET,JD_BITOFFSET,JD_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagModeButton)
    {
        info->event[info->num] = EVENT_ModeButton;
        info->num++;
        dataPoints->valueModeButton = gizStandardDecompressionValue(ModeButton_BYTEOFFSET,ModeButton_BITOFFSET,ModeButton_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagTempTH)
    {
        info->event[info->num] = EVENT_TempTH;
        info->num++;
        dataPoints->valueTempTH = gizX2Y(TempTH_RATIO,  TempTH_ADDITION, issuedData->attrVals.valueTempTH); 
    }
        
    if(0x01 == issuedData->attrFlags.flagTempLH)
    {
        info->event[info->num] = EVENT_TempLH;
        info->num++;
        dataPoints->valueTempLH = gizX2Y(TempLH_RATIO,  TempLH_ADDITION, issuedData->attrVals.valueTempLH); 
    }
        
    if(0x01 == issuedData->attrFlags.flagHumiHT)
    {
        info->event[info->num] = EVENT_HumiHT;
        info->num++;
        dataPoints->valueHumiHT = gizX2Y(HumiHT_RATIO,  HumiHT_ADDITION, issuedData->attrVals.valueHumiHT); 
    }
        
    if(0x01 == issuedData->attrFlags.flagHumiLT)
    {
        info->event[info->num] = EVENT_HumiLT;
        info->num++;
        dataPoints->valueHumiLT = gizX2Y(HumiLT_RATIO,  HumiLT_ADDITION, issuedData->attrVals.valueHumiLT); 
    }
        
    if(0x01 == issuedData->attrFlags.flagSoilHumiHT)
    {
        info->event[info->num] = EVENT_SoilHumiHT;
        info->num++;
        dataPoints->valueSoilHumiHT = gizX2Y(SoilHumiHT_RATIO,  SoilHumiHT_ADDITION, issuedData->attrVals.valueSoilHumiHT); 
    }
        
    if(0x01 == issuedData->attrFlags.flagSoilHumiLT)
    {
        info->event[info->num] = EVENT_SoilHumiLT;
        info->num++;
        dataPoints->valueSoilHumiLT = gizX2Y(SoilHumiLT_RATIO,  SoilHumiLT_ADDITION, issuedData->attrVals.valueSoilHumiLT); 
    }
        
    if(0x01 == issuedData->attrFlags.flagLightHT)
    {
        info->event[info->num] = EVENT_LightHT;
        info->num++;
        dataPoints->valueLightHT = gizX2Y(LightHT_RATIO,  LightHT_ADDITION, issuedData->attrVals.valueLightHT); 
    }
        
    if(0x01 == issuedData->attrFlags.flagLightLT)
    {
        info->event[info->num] = EVENT_LightLT;
        info->num++;
        dataPoints->valueLightLT = gizX2Y(LightLT_RATIO,  LightLT_ADDITION, issuedData->attrVals.valueLightLT); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueSPK != cur->valueSPK)
    {
        GIZWITS_LOG("valueSPK Changed\n");
        ret = 1;
    }
    if(last->valueJD != cur->valueJD)
    {
        GIZWITS_LOG("valueJD Changed\n");
        ret = 1;
    }
    ...
    if(last->valueModeButton != cur->valueModeButton)
    {
        GIZWITS_LOG("valueModeButton Changed\n");
        ret = 1;
    }
    if(last->valueTempTH != cur->valueTempTH)
    {
        GIZWITS_LOG("valueTempTH Changed\n");
        ret = 1;
    }
    if(last->valueTempLH != cur->valueTempLH)
    {
        GIZWITS_LOG("valueTempLH Changed\n");
        ret = 1;
    }
    if(last->valueHumiHT != cur->valueHumiHT)
    {
        GIZWITS_LOG("valueHumiHT Changed\n");
        ret = 1;
    }
    if(last->valueHumiLT != cur->valueHumiLT)
    {
        GIZWITS_LOG("valueHumiLT Changed\n");
        ret = 1;
    }
    if(last->valueSoilHumiHT != cur->valueSoilHumiHT)
    {
        GIZWITS_LOG("valueSoilHumiHT Changed\n");
        ret = 1;
    }
    if(last->valueSoilHumiLT != cur->valueSoilHumiLT)
    {
        GIZWITS_LOG("valueSoilHumiLT Changed\n");
        ret = 1;
    }
    if(last->valueLightHT != cur->valueLightHT)
    {
        GIZWITS_LOG("valueLightHT Changed\n");
        ret = 1;
    }
    if(last->valueLightLT != cur->valueLightLT)
    {
        GIZWITS_LOG("valueLightLT Changed\n");
        ret = 1;
    }
    ...
    if(last->valueSoilHumi != cur->valueSoilHumi)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueSoilHumi Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(SPK_BYTEOFFSET,SPK_BITOFFSET,SPK_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSPK);
    gizStandardCompressValue(JD_BYTEOFFSET,JD_BITOFFSET,JD_LEN,(uint8_t *)devStatusPtr,dataPoints->valueJD);
    gizStandardCompressValue(LED_BYTEOFFSET,LED_BITOFFSET,LED_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED);
    if(dataPoints->valueModeButton >= ModeButton_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueModeButton Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(ModeButton_BYTEOFFSET,ModeButton_BITOFFSET,ModeButton_LEN,(uint8_t *)devStatusPtr,dataPoints->valueModeButton);  
    }
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueTempTH = gizY2X(TempTH_RATIO,  TempTH_ADDITION, dataPoints->valueTempTH); 
    devStatusPtr->valueTempLH = gizY2X(TempLH_RATIO,  TempLH_ADDITION, dataPoints->valueTempLH); 
    devStatusPtr->valueHumiHT = gizY2X(HumiHT_RATIO,  HumiHT_ADDITION, dataPoints->valueHumiHT); 
    devStatusPtr->valueHumiLT = gizY2X(HumiLT_RATIO,  HumiLT_ADDITION, dataPoints->valueHumiLT); 
    devStatusPtr->valueSoilHumiHT = gizY2X(SoilHumiHT_RATIO,  SoilHumiHT_ADDITION, dataPoints->valueSoilHumiHT); 
    devStatusPtr->valueSoilHumiLT = gizY2X(SoilHumiLT_RATIO,  SoilHumiLT_ADDITION, dataPoints->valueSoilHumiLT); 
    devStatusPtr->valueLightHT = gizY2X(LightHT_RATIO,  LightHT_ADDITION, dataPoints->valueLightHT); 
    devStatusPtr->valueLightLT = gizY2X(LightLT_RATIO,  LightLT_ADDITION, dataPoints->valueLightLT); 
    ...
    devStatusPtr->valueSoilHumi = gizY2X(SoilHumi_RATIO,  SoilHumi_ADDITION, dataPoints->valueSoilHumi); 
    devStatusPtr->valueLight = gizY2X(Light_RATIO,  Light_ADDITION, dataPoints->valueLight); 
            ...
            else
            {
                if(1 == protocolFlag)
                {
                    ...
                    if(protocolCount > 4)
                    {
                        ...
                        if (tmpCount >= MAX_PACKAGE_LEN || tmpCount <= 4)
                        {
                            protocolFlag = 0;
                            protocolCount = 0;
                            GIZWITS_LOG("ERR:the data length is too long or too small, will abandon \n");
                        }
...
static int32_t gizProtocolGetDeviceInfo(protocolHead_t * head)
{
    ...
    deviceInfo.DataLen = exchangeBytes(GIZ_DATA_LEN);
    sprintf(deviceInfo.Data,"apName=%s&apPwd=%s&cfgMode=%s",AP_NAME,AP_PWD,CFG_MODE); 
#endif
...
{  
    ...
    memcpy((uint8_t *)&gizwitsProtocol.TimeNTP.year,(uint8_t *)UTTInfo->time, 7);
    memcpy((uint8_t *)&gizwitsProtocol.TimeNTP.ntp,(uint8_t *)UTTInfo->ntp_time, 4);
...
#if MODULE_TYPE
    memcpy((uint8_t *)&gizwitsProtocol.gprsInfoNews,(uint8_t *)&moduleInfo->gprsModuleInfo, sizeof(gprsInfo_t));
#else
